...
// POST /api/stories/supabase-add
export async function POST(req: NextRequest) {
  try {
    // Check if story exists
    const checkResponse = await fetch(
      `${SUPABASE_URL}/rest/v1/Story?id=eq.aziz-kazakhstan-oregon&select=id`,
      {
        method: 'GET',
        headers: {
    ...
    if (existing && existing.length > 0) {
      ...
      response = await fetch(
        `${SUPABASE_URL}/rest/v1/Story?id=eq.aziz-kazakhstan-oregon`,
        {
          method: 'PATCH',
          headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: `Failed: ${response.status} - ${error}` },
    ...
    return NextResponse.json({
      success: true,
      message: 'Story added/updated in Supabase successfully!',
      data: result,
    });
  } catch (error: any) {
