// Script to add/update story directly to Supabase
// Usage: npx tsx scripts/add-story-to-supabase.ts
...
async function addStoryToSupabase() {
  try {
    // Try to update first, if not exists, create
    const response = await fetch(`${SUPABASE_URL}/rest/v1/Story?id=eq.aziz-kazakhstan-oregon`, {
      method: 'PATCH',
      headers: {
    ...
    if (response.status === 404 || response.status === 204) {
      ...
      const createResponse = await fetch(`${SUPABASE_URL}/rest/v1/Story`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'apikey': SUPABASE_API_KEY,
      ...
      console.log('✅ Story created successfully:', created);
    } else if (!response.ok) {
      const error = await response.text();
      throw new Error(`Failed to update: ${response.status} - ${error}`);
    } else {
      const updated = await response.json();
      console.log('✅ Story updated successfully:', updated);
    }
  } catch (error: any) {
